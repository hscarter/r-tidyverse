...
# Data Formats: Wide versus Long

Today we will learn about two different data formats (or structures): wide and long. As in the previous sessions, we will provide code examples and exercises. But we want you first to understand very well the difference between these two ways of representing data. 

First, let´s check a very simple example in the following [statistics blog](https://www.statology.org/long-vs-wide-data/). What are the similarities and differences between both data frames?    

There are situations where we might want each row to be a different unit of observation.  We could make a really wide data set (one with lots of columns), where each row is a single ID column such as teams or countries and there are columns for any other measure. Or we make a data set that´s really long, where we have fewer columns but each row is a combination of different values (i.e., country-year). It will be long because we will have a repetition of countries and years. Or we could make something in between. 

These different configurations support different types of statistical models, data transformations, and data visualizations.  No configuration is necessarily the right one or better than the others in a vacuum - it depends on what you want to do with the data! And we talk about wide and long in relative terms.

# Pivot: from wide to long 

Today we want to show you how to transform a data frame from wide to long (and from long to wide). 

To begin with, we make a data set longer by reducing the number of columns and increasing the number of rows.  We'll stack some of the columns on top of each other, and duplicate the values in other columns to make this happen.  This changes what the unit of observation is (what each row represents).

## A Mini Example

Before we work with a larger data set, let's look at a very small one so it's easier to see what's happening. 
...
Now, we're going to work with a dataset that's part of the tidyr package: `who`
...
```{r}
sample_n(who, 10) #You can use a set.seed() for replication
```
...
#We can check different options in the help
```
...
pivot_wider(long_data, 
            names_from=diagnosis, #We will make columns from the distinct values of diagnosis. The old values in rows will become column names
            values_from=count) #We will fill the columns with the values in column
```
...
long_data %>%
  ...
  pivot_wider(names_from=diagnosis, 
              values_from=count,
              #values_fill=NA
              ) #We can fill NA with something else such as 0 
```
...
long_data %>%
  pivot_wider(names_from=c(age, gender),
              ...
              names_glue="{gender}_{age}", #We can get different combinations. "{gender}.{age} or something different such as age first"
              values_from=count) #We can go as wide as we want! We just need to change names_from and names_glue 
```
...
Hint: Make sure to run the code above for long_data
